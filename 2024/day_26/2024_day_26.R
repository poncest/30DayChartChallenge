...
## Challenge: #30DayChartChallenge 2024 day 26
## Topic:     Uncertainties | ai
## Author:    Steven Ponce
## Date:      2024-04-26

## Data:      Large Language Model List (via Wikipedia)
...
llm_data <- read_csv(here::here("2024/data/LLM List.csv")) |> 
  clean_names() |>
  glimpse()
...
### |- tidy data ---
llm_data_tidy <- llm_data |> 
  mutate(
    name = as_factor(name),
    developer = as_factor(developer),
    release_date = my(release_date),
  )

# label data
label_data <- plot_data |>
  filter(number_of_parameters_billion > 750)

### |- plot data ---
plot_data <- llm_data_tidy |>
  filter(!is.na(number_of_parameters_billion) & 
           !is.infinite(number_of_parameters_billion)) |>
  arrange(number_of_parameters_billion)
...
### |-  titles and caption ----
# icons
tt <- str_glue("#30DayChartChallenge: { 2024 } Day { 26 } &bull; Source: TidyTuesday 2021 wk 15<br>") 
li <- str_glue("<span style='font-family:fa6-brands'>&#xf08c;</span>")  
gh <- str_glue("<span style='font-family:fa6-brands'>&#xf09b;</span>")
mn <- str_glue("<span style='font-family:fa6-brands'>&#xf4f6;</span>")

# text
title_text    <- str_glue("Distribution of Parameters in Large Language Models (LLMs)") 

subtitle_text <- str_glue("Highlighting models with over 750 billion parameters, including PanGu-Σ and GLaM.")
...
theme_update(
  ...
  axis.text.x           = element_text(size = rel(.8), color = text_col, family = "text"),
...
### |-  Main plot ----  
plot_data |> 
  ggplot(aes(x = number_of_parameters_billion, y = factor(0))) +
  
  # Geom
  geom_boxplot(aes(fill = 'Distribution'), outliers = TRUE, width = 0.5) +
  geom_jitter(aes(color = 'Outliers'), width = 0, height = 0.12, alpha = 0.5) +
  
  ggforce::geom_mark_circle(
    data = label_data,
    aes(
      label = "Models with high number of parameters", 
      description = str_glue("PanGu-Σ by Huawei with 1,085B parameters  GLaM by Google with 1,200B parameters")
    ),
    expand = 0.05, 
  ...
  annotate(
    ...
    x = 110, 
    y = 0, 
    label = paste("Median =", median(plot_data$number_of_parameters_billion, na.rm = TRUE)), 
    vjust = -13, 
    color = "gray30"
  ) +
  ...
  # Scale
  scale_x_continuous(
    breaks = seq(0, 1300, by = 250),
    limits = c(0, 1300),
    labels = comma_format(suffix = " B")
  ) +
  ...
  # Labs
  labs(
    x = "Number of Parameters (billion)",
