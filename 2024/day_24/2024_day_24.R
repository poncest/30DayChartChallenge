...
## Challenge: #30DayChartChallenge 2024 day 24
## Topic:     Timeseries | ILO Region for Africa (data day)
## Author:    Steven Ponce
## Date:      2024-04-24 
...
#' [ILOSTAT explorer. Africa: Lower-middle income - Annual (2014 - 2024)]. 
#' Accessed [2024-04-24].
...
## 2. READ IN THE DATA ----
poverty_rate_data <- read_csv(here::here("2024/data/X08_A-full-2024-04-21.csv")) |>
  filter(
...
poverty_rate_data |> count(classif1_label, sort = T)
poverty_rate_data |> count(sex_label, sort = T)
poverty_rate_data |> count(time, sort = T)


## 3. EXAMINING THE DATA ----
glimpse(poverty_rate_data)
skim(poverty_rate_data)
...
### |- tidy data ---
poverty_rate_tidy <- poverty_rate_data |>
  select(-classif2_label) |>
  mutate(sex_label = str_remove(sex_label, pattern = "Sex: "))


### |- plot data ---
plot_data <- poverty_rate_tidy |>
  rename(
    year = time,
    pct = obs_value
  ) |>
  ...
  mutate(
    pct = pct / 100,
    age_bin = str_remove_all(classif1_label, pattern = "Age \\(Youth, adults\\): "),
    age_bin = factor(age_bin, levels = c("15+", "15-24", "25+"))
  )
...
caption_col  <- "gray20"   
text_col     <- "gray20"     
col_palette  <- moma.colors("Klein", type = "discrete", direction = 1)[c(1,2)]

### |-  titles and caption ----
# icons
tt <- str_glue("#30DayChartChallenge: { 2024 } Day { 24 } &bull; Source: ILOSTAT<br>") 
...
title_text    <- str_glue("Gender and Age Breakdown of Working Poverty in Africa (2000-2020)") 

subtitle_text <- str_glue("Comparative analysis of the working poverty rates among employed youth and adults,<br>
                          highlighting disparities between { female } and { male } groups living below USD $2.15 PPP")
...
### |-  final plot ----  
plot_data |>
  ggplot(aes(x = year, y = pct, group = sex_label, color = sex_label)) +
  ...
  labs(
    x = "Year",
    y = "Estimated Working Poverty Rate",
    title    = title_text,
