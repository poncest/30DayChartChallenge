...
## Challenge: #30DayChartChallenge 2024 day 09
## Topic:     Distributions | major/minor
## Author:    Steven Ponce
## Date:      2024-04-09

## Data:      US Accidents (2016 - 2023) (via Sobhan Moosavi)
##            A Countrywide Traffic Accident Dataset (2016 - 2023)
## Link:      https://www.kaggle.com/datasets/sobhanmoosavi/us-accidents/data

## Citation:   
#' Sobhan Moosavi. (2023). US Accidents (2016 - 2023) [Data set]. 
#' Kaggle. https://doi.org/10.34740/KAGGLE/DS/199387
...
## 2. READ IN THE DATA ----
raw_data <- data.table::fread(here::here("2024/data/US_Accidents_March23.csv")) |>
  clean_names() |>
  # Drop unwanted columns
  select(
    -c(end_time:street, zipcode),
    -c(airport_code:nautical_twilight),
  ) |>
  glimpse()


#' NOTE
#' If interested, download the data here: https://www.kaggle.com/datasets/sobhanmoosavi/us-accidents/data
...
raw_data |> count(state, sort = T)
raw_data |> count(severity, sort = T)
...
plot_data <- raw_data  |> 
  mutate(
    ...
    severity_scale = if_else(severity <= 2, "Minor Accident", "Major Accident")
  ) |>
  select(year, severity_scale, state, astronomical_twilight) |>
  filter(astronomical_twilight != "") |>
  count(severity_scale, state, astronomical_twilight, name = "count") |>
  arrange(desc(count))
...
caption_col  <- "gray20"  
text_col     <- "gray20"    
col_palette  <- c('Night' = '#355C7D', 'Day' = '#E76F51')


### |-  titles and caption ----
# icons
tt <- str_glue("#30DayChartChallenge: { 2024 } Day { 09 } &bull; Source: Kaggle via Sobhan Moosavi<br>")  
li <- str_glue("<span style='font-family:fa6-brands'>&#xf08c;</span>")  
gh <- str_glue("<span style='font-family:fa6-brands'>&#xf09b;</span>")
mn <- str_glue("<span style='font-family:fa6-brands'>&#xf4f6;</span>")

# text
title_text   <- str_glue("Traffic Accidents in the United States") 

day   <- str_glue("<span style='color:{ col_palette['Day'] }'>**Day**</span>")
night <- str_glue("<span style='color:{ col_palette['Night'] }'>**Night**</span>")

subtitle_text <- str_glue("Comparison of Major and Minor Car Accidents that Occurs During<br>
                          {day} or {night} Times from 2016 to 2023")
...
# Reference: https://github.com/eclarke/ggbeeswarm

plot_data |> 
  ggplot(aes(x = severity_scale, y = count, color = astronomical_twilight)) +
  
  # Geoms
  geom_beeswarm(cex = 2, size = 1.5) +
  annotate("text",
           x = "Minor Accident", y = 0, label = "Severity", vjust = -3, hjust = 1.5,
...
# P beeswarm      0.4.0      2021-06-01 [?] CRAN (R 4.3.1)
...
# P data.table  * 1.15.4     2024-03-30 [?] CRAN (R 4.3.3)
...
# P ggbeeswarm  * 0.7.2      2023-04-29 [?] CRAN (R 4.3.3)
...
# P vipor         0.4.7      2023-12-18 [?] CRAN (R 4.3.3)
