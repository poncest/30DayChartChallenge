...
## Challenge: #30DayChartChallenge 2024 day 06
## Topic:     Comparisons | OECD (data day)
## Author:    Steven Ponce
## Date:      2024-04-06

## Data:      Value added by activity (via OECD) 
## Link:      https://data.oecd.org/natincome/value-added-by-activity.htm

## Citation:  OECD (2024), Value added by activity (indicator). 
##            doi: 10.1787/a8b2bd2b-en (Accessed on 06 April 2024)
...
## 2. READ IN THE DATA ----
activity_2005_2022 <- read_xlsx(here::here("2024/data/OECD_Value_added_by_activity_2005-202.xlsx"),
...
glimpse(activity_2005_2022)
skim(activity_2005_2022)
colnames(activity_2005_2022)


## 4. TIDYDATA ----

### |- Tidy ----
activity_df <- activity_2005_2022 |>
  # Rename column
...
activity_combined <- bind_rows(activity_df, world_row)

data_plot <- activity_combined |> 
  # Pivot longer
  pivot_longer(
    cols = -country, 
    names_to = "year", 
...
### |-  titles and caption ----
# icons
tt <- str_glue("#30DayChartChallenge: { 2024 } Day { 06 } &bull; Source: OECD National Accounts at a Glance<br>")  
li <- str_glue("<span style='font-family:fa6-brands'>&#xf08c;</span>")  
gh <- str_glue("<span style='font-family:fa6-brands'>&#xf09b;</span>")
mn <- str_glue("<span style='font-family:fa6-brands'>&#xf4f6;</span>")

# text
title_text    <- str_glue("Value Added by Activity, 2005 - 2022") 

subtitle_text <- str_glue("Value added by activity is the value created by industries (such as agriculture, industry, utilities, and other service activities),<br>
                          measured as the value of output minus the value of intermediate consumption. It reflects the value generated by producing<br>
                          goods and services.")
...
theme_update(
  ...
  axis.line.x           = element_line(color = "gray10", linewidth = .2),
...
highlighted_countries <- c("India", "Indonesia", "United States", "China", "World")

data_plot |>
  ggplot(aes(x = year, y = pct, group = country)) +
  # Gray lines for countries not highlighted
  geom_line(
    data = data_plot |> filter(!country %in% highlighted_countries),
    color = "gray", size = gray_lines
  ) +
  # Generate lines and labels for highlighted countries
  lapply(1:length(highlighted_countries), function(i) {
    country_name <- highlighted_countries[i]
    list(
      geom_line(
        data = data_plot |> filter(country == country_name),
        aes(color = country), size = highlight_lines
      ),
      geomtextpath::geom_textline(
        data = data_plot |> filter(country == country_name),
        aes(label = country, color = country),
        family = "text", size = 4, fontface = "bold",
        offset = unit(0.15, "cm"), text_smoothing = 30
      )
    )
  }) +
  ...
  scale_color_manual(values = setNames(col_palette, highlighted_countries)) +
...
# P geomtextpath * 0.1.3      2024-03-12 [?] CRAN (R 4.3.3)
