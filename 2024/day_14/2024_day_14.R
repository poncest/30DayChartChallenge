...
## Challenge: #30DayChartChallenge 2024 day 14
## Topic:     Relationships | heatmap
## Author:    Steven Ponce
## Date:      2024-04-14
...
plot_data <- steps_data_tidy |> 
  ...
  mutate(                                                                       # Bins
    steps_bin = case_when(
      value <= 5000  ~ "5 or less",
      value <  10000  ~ "5 - 10",
      value <  15000  ~ "10 - 15",
      value <  20000  ~ "15 - 20",
      value >= 20000  ~ "20 or more"
    ),
    steps_bin = factor(steps_bin,                                               # Factors
                       levels = c("5 or less", "5 - 10", "10 - 15","15 - 20", "20 or more")
    )
  )
...
### |- plot aesthetics ---- 
bkg_col      <- colorspace::lighten('#252525', 0.05)   
title_col    <- "#d8eef7"            
subtitle_col <- "#d8eef7"                 
caption_col  <- "#d8eef7"               
text_col     <- "#d8eef7"                 
col_palette  <- colorspace::lighten("#00ff89",  seq(.8, 0, length.out = 5)) 

### |-  titles and caption ----
# icons
tt <- str_glue("#30DayChartChallenge: { 2024 } Day { 14 } &bull; Source: Pedometer++<br>") 
li <- str_glue("<span style='font-family:fa6-brands'>&#xf08c;</span>")  
gh <- str_glue("<span style='font-family:fa6-brands'>&#xf09b;</span>")
mn <- str_glue("<span style='font-family:fa6-brands'>&#xf4f6;</span>")

# text
title_text    <- str_glue("Annual Step Count Analysis") 

subtitle_text <- str_glue("Comparative Heatmap of Minimum, Mean, and Maximum Steps<br>
                          Recorded Yearly (2018-2024)")

fill_text     <- "Steps Bins (K)"
...
### |-  final plot ----  
plot_data |>
  ggplot(aes(x = year, y = str_to_title(statistic), fill = steps_bin)) +

  # Geoms
  geom_tile(color = "gray50") +

  # Scales
  scale_x_continuous(breaks = seq(2018, 2024, by = 1), position = "top") +
  scale_fill_manual(values = col_palette, expand = c(0, 0)) +
  guides(fill = guide_legend(title.position = "top", nrow = 1)) +
  coord_equal(clip = "off", ratio = 0.5)+ 
