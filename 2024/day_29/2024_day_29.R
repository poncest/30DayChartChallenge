...
## Challenge: #30DayChartChallenge 2024 day 29
## Topic:     Uncertainties | black & white
## Author:    Steven Ponce
## Date:      2024-04-29

## Data:      Hollywood Age Gaps 
##            TidyTuesday 2023 week 07 (2023-02-14)
## Link:      https://github.com/rfordatascience/tidytuesday/blob/master/data/2023/2023-02-14/readme.md
...
# Data from tidytuesday 2023 week 07 "Hollywood Age Gaps' (2023-02-14).
...
jb_movies_list <- c(
  ...
  "Diamonds Are Forever", "Live and Let Die", "The Man with the Golden Gun", "The Spy Who Loved Me", "Moonraker",
  "For Your Eyes Only", "Octopussy", "A View to a Kill", "The Living Daylights", "Licence to Kill", "GoldenEye",
  "Tomorrow Never Dies", "The World Is Not Enough", "Die Another Day", "Casino Royale", "Quantum of Solace", "Skyfall",
...
jb_movies_tbl <- age_gaps |>
  ...
  mutate(
    decade = paste0((release_year - 1) %/% 10 * 10, "'s"),
    movie_name = fct_reorder(movie_name, release_year, .desc = TRUE),
    year_movie_title = str_glue("{movie_name}, {release_year}"),
    year_movie_title = fct_reorder(year_movie_title, release_year, .desc = TRUE)
  )

# Mean age difference by decade   
...
data_plot <- jb_movies_tbl |>
  ...
  mutate(
    highlight = ifelse(
      age_difference > mean_age_diff, "Above Average", "Below Average"
    )
  )
...
caption_col  <- "#333333"    
text_col     <- "#333333"   
col_palette  <- c("Below Average" = "#111111", "Above Average" = "#111111")  
shapes       <- c("Below Average" = 21, "Above Average" = 16) 


### |-  titles and caption ----
# icons
tt <- str_glue("#30DayChartChallenge: { 2024 } Day { 29 } &bull; Source: 2023 TidyTuesday wk 07<br>") 
...
font_add_google("Alata", regular.wt = 400, family = "title")                 
font_add_google("Alata", regular.wt = 400, family = "subtitle")  
font_add_google("Roboto Condensed", regular.wt = 400, family = "text")        
font_add_google("PT Sans Narrow", regular.wt = 400,family = "caption")
...
theme_update(
  ...
  strip.text.y          = element_text(size = rel(1.1), face = "bold", color = text_col, family = "text"),
...
p <- data_plot |> 
  ggplot(aes(x = movie_name, y = age_difference, color = highlight, shape = highlight)) + 
...
arrowdf <- tibble(
  decade = "1970's",
  label_text = "Mean age difference for each decade",
)

p +
  geom_segment(
    data = arrowdf,
    aes(x = 5.5, xend = 4.5, y = 25, yend = 20.090909), 
    inherit.aes = FALSE,
    color = "#888888",
    size = 0.8,
    alpha = 0.9,
  ...
  geom_text(
    data = arrowdf,
    aes(x = 6.4, y = 29, label = str_wrap(label_text, width = 20)), 
    inherit.aes = FALSE,
    color = "#888888"
  )
