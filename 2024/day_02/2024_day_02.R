...
## Challenge: #30DayChartChallenge 2024 day 02
## Topic:     Comparison | neo
## Author:    Steven Ponce
## Date:      2024-04-02
...
worldwide_hydrogen_projects |> count(country, sort = T)
worldwide_hydrogen_projects |> count(product, sort = T)
worldwide_hydrogen_projects |> count(status, sort = T)
worldwide_hydrogen_projects |> count(technology, sort = T)
worldwide_hydrogen_projects |> count(technology_electricity, sort = T)
...
h2_usa_tbl <- worldwide_hydrogen_projects |>
  ...
  filter(
    product == "H2",
    country == "USA",
    !is.na(capacity_nm3_h2_h)
  ) |>
  # Summary
  group_by(status) |>
  summarise(
    count = n(),
    avg_h2_capacity = mean(capacity_nm3_h2_h)
  ) |>
  ungroup() |>
  # Format and reorder
  mutate(
    status = case_when(
      status == "DEMO" ~ "Demo",
      TRUE ~ as_factor(status)
    ),
    status = fct_reorder(status, avg_h2_capacity)
  ) |>
  # Bar labels
  mutate(bar_labels = scales::number_format(
    big.mark = ", ",
    accuracy = 1
  )
  (avg_h2_capacity))
...
caption_col  <- "#4C4D4D"
text_col     <- "#4C4D4D"     
col_palette  <- c("#1aad74")


### |-  titles and caption ----
# icons
tt <- str_glue("#30DayChartChallenge: { 2024 } Day { 02 } &bull; Source: The Hydrogen Production Projects Database<br>")  
li <- str_glue("<span style='font-family:fa6-brands'>&#xf08c;</span>")  
gh <- str_glue("<span style='font-family:fa6-brands'>&#xf09b;</span>")
mn <- str_glue("<span style='font-family:fa6-brands'>&#xf4f6;</span>")

# text
title_text    <- str_glue("Hydrogen Production Projects in the USA") 

subtitle_text <- str_glue("Out of the total 97 projects, only 15 projects are currently operational.<br><br>",
                          "<span style='font-size:9pt; color:#4C4D4D'> Mean hydrogen generation&nbsp;(Nm³H₂/h) through various technologies.</span>")
...
font_add_google("Montserrat Alternates", regular.wt = 600, family = "title")       
font_add_google("Questrial", regular.wt = 400, family = "subtitle")  
font_add_google("Questrial", regular.wt = 400, family = "text")  
...
theme_update(
  ...
  axis.line.y           = element_line(color = "gray90", linewidth = .2),
  panel.grid            = element_blank(),
)

### |-  create_bar_plot() ----
create_bar_plot <- function(tbl, x_variable, y_variable, label_adjust, col_width) {
  tbl |>
    ggplot(aes(x = {{ x_variable }}, y = {{ y_variable }})) +
    
    # Geoms
    geom_col(width = {{ col_width }}, fill = col_palette[1]) +
    geom_textbox(
      aes(
        label = paste0(
        ...
        hjust = case_when(
          avg_h2_capacity < {{ label_adjust }} ~ 0,
          TRUE ~ 1
        ),
        halign = case_when(
          avg_h2_capacity < {{ label_adjust }} ~ 0,
        ...
        colour = case_when(
          avg_h2_capacity < {{ label_adjust }} ~ "#4C4D4D",
          TRUE ~ "#fdfdff"
        )
      ),
      fill = NA,
