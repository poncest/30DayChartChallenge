...
## Challenge: #30DayChartChallenge 2024 day 22
## Topic:     Timeseries | mobility
## Author:    Steven Ponce
## Date:      2024-04-22

## Data:      Google Trends
##            Shoulder vs. Hip Mobility Terms
## Link:      https://trends.google.com/trends/explore?date=all&geo=US&q=shoulder%20mobility,hip%20mobility&hl=en

## Citation:
#' Google Trends. Shoulder &. Hip Mobility Terms | 2004 - present | All categories | Web Search
#' [Google Trends. Shoulder & Hip Mobility Terms. (2004 - 2024)]. 
#' Accessed [2024-04-22].
...
## 2. READ IN THE DATA ----
mobility_data_2004_2024 <- read_csv(here::here("2024/data/google_trend_shoulder_hip_mobility.csv"),
  skip = 2,
  trim_ws = TRUE,
) |>
  clean_names() |>
...
glimpse(mobility_data_2004_2024)
skim(mobility_data_2004_2024)
colnames(mobility_data_2004_2024)


## 4. TIDYDATA ----

### |- tidy data ---
mobility_tidy <- mobility_data_2004_2024 |>
  mutate(
    date = ym(month),
  ...
  rename(
    shoulder = shoulder_mobility_united_states,
    hip      = hip_mobility_united_states
  ) |>
  select(date, month, year, everything())

### |- plot data ---
plot_data <- mobility_tidy |>
  pivot_longer(
...
# 5. VISUALIZATION ---- 
### |- plot aesthetics ---- 
bkg_col      <- colorspace::lighten('brown', 0.98) 
title_col    <- "gray10"             
subtitle_col <- "gray10"     
caption_col  <- "gray20"   
text_col     <- "gray20"     
col_palette  <- MoMAColors::moma.colors("Doughton", n = 2, type = "discrete")


### |-  titles and caption ----
# icons
tt <- str_glue("#30DayChartChallenge: { 2024 } Day { 22 } &bull; Source: Google Trends<br>") 
li <- str_glue("<span style='font-family:fa6-brands'>&#xf08c;</span>")  
gh <- str_glue("<span style='font-family:fa6-brands'>&#xf09b;</span>")
mn <- str_glue("<span style='font-family:fa6-brands'>&#xf4f6;</span>")

# text
hip           <- str_glue("<span style='color:{ col_palette[1] }'>**Hip**</span>")
shoulder      <- str_glue("<span style='color:{ col_palette[2] }'>**Shoulder**</span>")

title_text    <- str_glue("Trends in { hip } vs. { shoulder } Mobility: A Google Search Perspective") 

subtitle_text <- str_glue("An analysis of Google search trends reveals the growing public interest in<br>
                          hip and shoulder mobility from 2004 to 2024")
...
### |-  final plot ----  
plot_data |> 
  ggplot(aes(x = year, y = score, color = mobility_type)) + 
  
  # Geoms
  geom_smooth(linewidth = 0.5, method = NULL) +
